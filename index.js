...
async function run() {
    try {
        ...
        app.post("/upcomming-meals", async (req, res) => {
            ...
            if (!filter) {
                const result = await upcommingMealsCollection.insertOne(upcommingMealsData);
                if (result.insertedId) {
                    res.send({ success: true, message: "Upcomming meals added successfully" })
                } else {
                    res.send({ success: false, message: "Something went wrong" })
                }
            } else {
        ...
        app.patch("/upcomming-meals/:id/like", async (req, res) => {
            ...
            const meal = await upcommingMealsCollection.findOne({ _id: new ObjectId(mealId) });
            ...
            if (meal.likedBy && meal?.likedBy?.includes(userId)) {
                return res.status(400).send({ error: "You have already liked this meal" })
            }
            ...
            const result = await upcommingMealsCollection.updateOne(filter, updatedDoc);
            res.send(result);
        });
        ...
        app.patch("/upcomming-meals/:id/dicrease", async (req, res) => {
            ...
            const meal = await upcommingMealsCollection.findOne({ _id: new ObjectId(mealId) });
            if (!meal) {
                return res.status(400).send({ error: "Meal not found on the collection" })
            }

            if (meal.likedBy && !meal.likedBy.includes(userId)) {
                return res.status(400).send({ error: "You don't have permission to dicrease like" })
            }
